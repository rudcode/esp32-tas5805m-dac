...
#pragma message( \
    "tas5805m_2.0+eq(+9db_20Hz)(-1Db_500Hz)(+3Db_8kHz)(+3Db_15kHz) config is used")
...
#pragma message( \
    "tas5805m_2.0+eq(+9db_20Hz)(-3Db_500Hz)(+3Db_8kHz)(+3Db_15kHz) config is used")
...
#pragma message( \
    "tas5805m_2.0+eq(+12db_30Hz)(-3Db_500Hz)(+3Db_8kHz)(+3Db_15kHz) config is used")
  ...
  while (i < size) {
    switch (conf_buf[i].offset) {
      ...
      case CFG_META_DELAY:
        ...
        break;
      case CFG_META_BURST:
        ret = tas5805m_write_bytes((unsigned char *)(&conf_buf[i + 1].offset),
                                   1, (unsigned char *)(&conf_buf[i + 1].value),
                                   conf_buf[i].value);
      ...
      default:
        ...
        ESP_LOGV(TAG, "\t0x%02x <- 0x%02x", conf_buf[i].offset,
                 conf_buf[i].value);
        break;
    }
    i++;
...
esp_err_t tas5805m_init() {
  ...
  ESP_LOGI(TAG, "%s: Running PWR ON sequence on pin: %d", __func__,
           TAS5805M_GPIO_PDN);
  gpio_config(&io_conf);
  ...
  // Once I2S clocks are stable, set the device into HiZ state and enable DSP
  // via the I2C control port.
  ESP_ERROR_CHECK(tas5805m_set_state(TAS5805M_CTRL_HI_Z));
  vTaskDelay(20 / portTICK_RATE_MS);
  // 5. Wait 5ms at least. Then initialize the DSP Coefficient, then set the
  // device to Play state.
...
esp_err_t tas5805m_set_volume_pct(uint8_t vol) {
  esp_err_t ret = ESP_OK;
  if (vol < TAS5805M_VOLUME_PCT_MIN) vol = TAS5805M_VOLUME_PCT_MIN;
  if (vol > TAS5805M_VOLUME_PCT_MAX) vol = TAS5805M_VOLUME_PCT_MAX;
...
esp_err_t tas5805m_set_eq(bool enable) {
  ...
  return tas5805m_write_byte(
      TAS5805M_DSP_MISC, enable ? TAS5805M_CTRL_EQ_ON : TAS5805M_CTRL_EQ_OFF);
}
...
esp_err_t tas5805m_set_eq_gain(int band, int gain) {
  ...
  int current_page = 0;
  int ret = ESP_OK;
  ESP_LOGD(TAG, "%s: Setting EQ band %d (%d Hz) to gain %d", __func__, band,
           tas5805m_eq_bands[band], gain);
  ...
  for (int i = 0; i < TAS5805M_EQ_KOEF_PER_BAND * TAS5805M_EQ_REG_PER_KOEF;
    ...
    if (ret != ESP_OK) {
      ESP_LOGE(TAG, "%s: Error writing to register 0x%x", __func__,
               reg_value->offset);
    }
  }
...
esp_err_t tas5805m_set_modulation_mode(TAS5805M_MOD_MODE mode,
  ...
  ESP_LOGD(
      TAG,
      "%s: Setting modulation to %d, FSW: %d, Class-D bandwidth control: %d",
      __func__, mode, freq, bd_freq);
...
esp_err_t tas5805m_set_mixer_mode(TAS5805M_MIXER_MODE mode) {
  ...
  TAS5805M_SET_BOOK_AND_PAGE(TAS5805M_REG_BOOK_5,
                             TAS5805M_REG_BOOK_5_MIXER_PAGE);
  ret = tas5805m_write_byte(TAS5805M_REG_LEFT_TO_LEFT_GAIN, mixer_l_to_l);
  if (ret != 0) {
    ESP_LOGE(TAG, "Failed to write register %d: %d",
             TAS5805M_REG_LEFT_TO_LEFT_GAIN, ret);
    return ret;
  }
  ...
  if (ret != 0) {
    ESP_LOGE(TAG, "Failed to write register %d: %d",
             TAS5805M_REG_RIGHT_TO_RIGHT_GAIN, ret);
    return ret;
  }
  ...
  if (ret != 0) {
    ESP_LOGE(TAG, "Failed to write register %d: %d",
             TAS5805M_REG_LEFT_TO_RIGHT_GAIN, ret);
    return ret;
  }
  ...
  if (ret != 0) {
    ESP_LOGE(TAG, "Failed to write register %d: %d",
             TAS5805M_REG_RIGHT_TO_LEFT_GAIN, ret);
    return ret;
  }
...
void tas5805m_decode_faults(TAS5805M_FAULT fault) {
  ...
  if (fault.err1) {
    if (fault.err1 & (1 << 0)) ESP_LOGW(TAG, "%s: PVDD UV fault", __func__);

    if (fault.err1 & (1 << 1)) ESP_LOGW(TAG, "%s: PVDD OV fault", __func__);

    if (fault.err1 & (1 << 2)) ESP_LOGW(TAG, "%s: Clock fault", __func__);
