...
class CommandLine {
...
#ifdef CONFIG_DAC_TAS5805M
      new AmpStateCommand(), new VolumeCommand(), new Volume100Command(),
      new DacModeCommand(),  new EqCommand(),     new ModulationCommand(),
      new GainCommand(),     new MixerCommand(),  new FaultCommand(),
#endif
  };
 ...
 public:
  ...
  esp_err_t addCommand(char *name, char *help,
                       int (*func)(int argc, char **argv)) {
    esp_console_cmd_t cmd = {.command = name,
                             .help = help,
                             .hint = NULL,
                             .func = func,
                             .argtable = NULL};
  ...
  void init() {
    ...
    ESP_LOGI(TAG,
             "ESP32 Command Line Interface Ready! Type 'help' for a list of "
             "commands.");
  }
  ...
  void startLoopAsync() {
    if (taskHandle == NULL) {
      ...
      xTaskCreatePinnedToCore(&taskLoop,      // Task function
  ...
  static void loop() { process_serial_commands(); };
};
