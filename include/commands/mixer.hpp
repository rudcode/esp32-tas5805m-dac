...
class MixerCommand : public Command {
 ...
 private:
  ...
  static MixerMode get_mixer_mode(const char *arg) {
    if (strcmp(arg, "stereo") == 0) return STEREO;
    if (strcmp(arg, "inv") == 0) return INV;
    if (strcmp(arg, "mono") == 0) return MONO;
    if (strcmp(arg, "left") == 0) return LEFT;
    if (strcmp(arg, "right") == 0) return RIGHT;
    return UNKNOWN;
  }
 ...
 public:
  ...
  static inline MixerArgs mixer_args = {
      arg_str0(NULL, NULL, "[stereo|inv|mono|left|right]",
               "Mixer mode, one of the listed"),
      arg_end(1)};

  String getName() { return "mixer"; };

  esp_console_cmd_t getCommand() {
    return {.command = "mixer",
