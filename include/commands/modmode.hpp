...
class ModulationCommand : public Command {
 ...
 private:
  ...
  typedef enum { MOD_BD, MOD_1SPW, MOD_HYBRID, MOD_INVALID } mod_mode_t;

  static mod_mode_t get_mod_mode(const char *arg) {
    if (strcmp(arg, "bd") == 0) return MOD_BD;
    if (strcmp(arg, "1spw") == 0) return MOD_1SPW;
    if (strcmp(arg, "hybrid") == 0) return MOD_HYBRID;
    return MOD_INVALID;
  }
  ...
  typedef enum { SW_768K, SW_384K, SW_480K, SW_576K, SW_INVALID } sw_freq_t;

  static sw_freq_t get_sw_freq(const char *arg) {
    if (strcmp(arg, "768k") == 0) return SW_768K;
    if (strcmp(arg, "384k") == 0) return SW_384K;
    if (strcmp(arg, "480k") == 0) return SW_480K;
    if (strcmp(arg, "576k") == 0) return SW_576K;
    return SW_INVALID;
  }
  ...
  static bd_freq_t get_bd_freq(const char *arg) {
    if (strcmp(arg, "80k") == 0) return BD_FREQ_80K;
    if (strcmp(arg, "100k") == 0) return BD_FREQ_100K;
    if (strcmp(arg, "120k") == 0) return BD_FREQ_120K;
    if (strcmp(arg, "175k") == 0) return BD_FREQ_175K;
  ...
  static int mod_command_handler(int argc, char **argv) {
    ...
    if (mod_args.mode->count == 0) {
      ...
      ESP_LOGI(TAG,
               "Current modulation mode is %s, dsp processing freq: %s, output "
               "switching freq: %s",
               mod_mode_to_string(mode), sw_freq_to_string(freq),
               bd_freq_to_string(bd_freq));
      return 0;
    }
    ...
    if (_freq == SW_INVALID) {
      ESP_LOGE(TAG, "%s: Invalid frequency %s", __func__,
               mod_args.freq->sval[0]);
      return ESP_ERR_INVALID_ARG;
    }

    bd_freq_t _bd_freq = get_bd_freq(mod_args.bd_freq->sval[0]);
    if (_bd_freq == BD_FREQ_INVALID) {
      ESP_LOGE(TAG, "%s: Invalid bd frequency %s", __func__,
               mod_args.bd_freq->sval[0]);
      return ESP_ERR_INVALID_ARG;
    }
    ...
    ESP_LOGI("CMD",
             "Setting modulation mode to %s, dsp processing freq: %s, "
             "switching freq: %s",
             mod_mode_to_string(mode), sw_freq_to_string(freq),
             bd_freq_to_string(bd_freq));
