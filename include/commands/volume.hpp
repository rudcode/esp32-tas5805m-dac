...
class VolumeCommand : public Command {
 private:
  ...
  static int volume_command_handler(int argc, char **argv) {
    ...
    if (args.volume->count == 0) {
      ...
      ESP_LOGI(TAG, "Current volume is %d, which is %f Db", volume,
               volume_to_db(volume));
      return 0;
    }
    ...
    if (volume < TAS5805M_VOLUME_MIN || volume > TAS5805M_VOLUME_MAX) {
      ESP_LOGI(TAG, "Invalid volume level! Must be between %d and %d.",
               TAS5805M_VOLUME_MIN, TAS5805M_VOLUME_MAX);
      return 1;
    }

    ESP_LOGI(TAG, "Setting volume to %d% [%d..%d], which is %f Db", volume,
             TAS5805M_VOLUME_MIN, TAS5805M_VOLUME_MAX, volume_to_db(volume));
 ...
 public:
  ...
  static inline VolumeArgs volume_args = {
      arg_int0(NULL, NULL, "[volume]",
               "Volume level [0..255], default 48 (+0 Db)"),
      arg_end(1)};

  String getName() { return "volume"; };

  esp_console_cmd_t getCommand() {
    return {
        .command = "volume",
        .help =
            "Set the volume level of the amp. Usage: volume [volume], where "
            "volume is between 0 (+24 Db) and 255 (Mute), default 28 (+O db).",
        .hint = NULL,
