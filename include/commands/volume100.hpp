...
class Volume100Command : public Command {
  ...
  static int volume_command_handler(int argc, char **argv) {
    ...
    if (volume < TAS5805M_VOLUME_PCT_MIN || volume > TAS5805M_VOLUME_PCT_MAX) {
      ESP_LOGI(TAG, "Invalid volume level! Must be between %d and %d.",
               TAS5805M_VOLUME_PCT_MIN, TAS5805M_VOLUME_PCT_MAX);
      return 1;
    }
 ...
 public:
  ...
  static inline VolumeArgs volume_args = {
      arg_int0(NULL, NULL, "[vol]",
               "Volume level, percent [0..124], default 100 (+0 Db)"),
      arg_end(1)};

  String getName() { return "vol"; };

  esp_console_cmd_t getCommand() {
    return {.command = "vol",
            .help =
                "Set the volume percent level of the amp. Usage: volume "
                "[volume], where volume is between 0 (Mute) and 124 (+24Db), "
                "default 100 (+O db).",
            .hint = NULL,
