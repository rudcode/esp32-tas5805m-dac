...
class DacModeCommand : public Command {
 private:
  ...
  // Enum for DAC modes
  typedef enum { DAC_MODE_BTL, DAC_MODE_PBTL, DAC_MODE_INVALID } dac_mode_t;

  // Function to parse DAC mode from string
  static dac_mode_t get_dac_mode(const char *arg) {
    if (strcmp(arg, "btl") == 0) return DAC_MODE_BTL;
    if (strcmp(arg, "pbtl") == 0) return DAC_MODE_PBTL;
  ...
  static int dacmode_command_handler(int argc, char **argv) {
    ...
    if (args.mode->count == 0) {
      ...
      ESP_LOGI(TAG, "Current DAC mode is %s",
 ...
 public:
  ...
  static inline DacModeArgs dacmode_args = {
      arg_str0(NULL, NULL, "[mode]", "DAC mode: btl, pbtl"), arg_end(1)};

  String getName() { return "dacmode"; }

  esp_console_cmd_t getCommand() {
    return {.command = "dacmode",
