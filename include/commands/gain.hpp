...
class GainCommand : public Command {
 ...
 private:
  static float gain_to_db(int gain_ix) { return -gain_ix / 2.0; }
  ...
  static int gain_command_handler(int argc, char **argv) {
    ...
    if (gain_args.gain->count == 0) {
      ...
      ESP_LOGI(TAG, "Current gain index is %d, which is %f Db", gain,
               gain_to_db(gain));
      return 0;
    }
    ...
    ESP_LOGI(TAG, "Setting gain to %d, which is %f Db", gain_ix,
             gain_to_db(gain_ix));
 ...
 public:
  ...
  static inline GainArgs gain_args = {
      arg_int0(NULL, NULL, "<gain>",
               "Gain level, 0..31, 0 = 0dB, 31 = -15.5dB"),
      arg_end(1)};

  String getName() { return "gain"; };
