...
class tas5805m {
 ...
 public:
  ...
  esp_err_t init() { return tas5805m_init(); };

  esp_err_t getVolume(uint8_t *vol) { return tas5805m_get_volume(vol); };

  esp_err_t setVolume(uint8_t vol) { return tas5805m_set_volume(vol); };

  esp_err_t getVolume100(uint8_t *vol) { return tas5805m_get_volume_pct(vol); };

  esp_err_t setVolume100(uint8_t vol) { return tas5805m_set_volume_pct(vol); };
  ...
  esp_err_t getEqEnabled(bool *enabled) { return tas5805m_get_eq(enabled); };

  esp_err_t setEqEnabled(bool enabled) { return tas5805m_set_eq(enabled); };
  ...
  esp_err_t getModulationMode(TAS5805M_MOD_MODE *mode, TAS5805M_SW_FREQ *freq,
                              TAS5805M_BD_FREQ *bd_freq) {
    return tas5805m_get_modulation_mode(mode, freq, bd_freq);
  };

  esp_err_t setModulationMode(TAS5805M_MOD_MODE mode, TAS5805M_SW_FREQ freq,
                              TAS5805M_BD_FREQ bd_freq) {
    return tas5805m_set_modulation_mode(mode, freq, bd_freq);
  };

  esp_err_t getAnalogGain(uint8_t *gain) { return tas5805m_get_again(gain); };

  esp_err_t setAnalogGain(uint8_t gain) { return tas5805m_set_again(gain); };
  ...
  esp_err_t clearFaultState() { return tas5805m_clear_faults(); };

  void decodeFaults(TAS5805M_FAULT fault) { tas5805m_decode_faults(fault); }
};